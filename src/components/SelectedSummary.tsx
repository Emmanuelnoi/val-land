  ...
  return (
    <Card className="bg-white/80">
      <div className="flex items-center justify-between">
        ...
        <span className="rounded-full bg-accent-soft-strong px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] tabular-nums text-accent-strong">
        ...
        {selected.length === 0 ? (
          ...
          selected.map((gift) => (
            <div key={gift.id} className="flex min-w-0 items-center gap-3 text-sm text-ink-400">
              <span className="flex h-9 w-9 items-center justify-center rounded-full bg-accent-soft text-accent shadow-soft">
              ...
              {onRemove ? (
                <button
                  type="button"
                  className="ml-auto inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold text-accent transition-colors hover-accent-soft hover-text-accent-strong focus-ring touch-manipulation"
